...
import org.leocoder.picture.utils.ColorSimilarUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public List<PictureVO> searchPictureByColor(Long spaceId, String picColor, User loginUser) {
        // 1. 校验参数
        ThrowUtils.throwIf(ObjectUtil.isNull(spaceId) || StrUtil.isBlank(picColor), ErrorCode.PARAMS_ERROR);
    ...
    @Override
    public void editPictureByBatch(PictureEditByBatchRequest requestParam, User loginUser) {
        List<Long> pictureIdList = requestParam.getPictureIdList();
        Long spaceId = requestParam.getSpaceId();
        String category = requestParam.getCategory();
        List<String> tags = requestParam.getTags();

        // 1. 校验参数
        ThrowUtils.throwIf(ObjectUtil.isNull(spaceId) || CollUtil.isEmpty(pictureIdList), ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(ObjectUtil.isNull(loginUser), ErrorCode.NO_AUTH_ERROR);
        // 2. 校验空间权限
        Space space = spaceService.getById(spaceId);
        ThrowUtils.throwIf(ObjectUtil.isNull(space), ErrorCode.NOT_FOUND_ERROR, "空间不存在");
        ...
        // 批量重命名
        String nameRule = requestParam.getNameRule();
